...
def mlab_plt_activated(S_overl):##############################################################
	...
	# barrier_plot = mlab.contour3d(plot_output.detach().numpy(), contours = [-superp.TOL_BOUNDARY, 0, -superp.TOL_BOUNDARY], \
	# 								color = (1, 1, 0), opacity = 0.3) # yellow
	#src = mlab.pipeline.scalar_field(plot_input.detach().numpy())
	#barrier_plot = mlab.pipeline.iso_surface(src, contours = [-superp.TOL_BOUNDARY, 0, superp.TOL_BOUNDARY],color = (1, 1, 0), opacity = 1)
...
def plot_act_3d(S_overl):
	mlab.figure(fgcolor = (0, 0, 0), bgcolor = (1, 1, 1))
	mlab_plt_activated(S_overl)
	mlab_plt_init()
	mlab_plt_unsafe()
	mlab_plt_vector()
	mlab_plt_flow()
